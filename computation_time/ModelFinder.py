...
for files in os.listdir(path_of_the_directory):
    if files.endswith(ext):
       if 'F81' in files:
           start = time.time()
           for i in range(1000):
               command = "iqtree2.1.3 -mset JC,K2P,F81,F84,HKY,GTR -redo -s " + files + ' -m MF'
           ...
           start = time.time()
           for i in range(1000):
               command = "iqtree2.1.3 -mset JTT,LG,WAG,Dayhoff -redo -s " + files + ' -m MF'
               subprocess.run([command], shell=True)
           end = time.time()
           total_time = end - start
           f.write('File name: ')
